...
if (!text) throw `ضع اسم دولتك بي الإنجليزية متال: \n ${usedPrefix + command} sudan`
    try {
...
async function fetchPrayerTimes(q) {
  ...
  try {
    ...
    const prayerTimes = {
      تاريخ: $('b:contains("الإثنين")').text().trim(), // Mengambil informasi tanggal adzan saat ini
